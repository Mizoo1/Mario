...
namespace Monster.GUI.Sprite
{
    public class Robot : Enemy
    {
        ...
        public Robot(IntPtr _texture) : base(_texture, 0, 0, 50, 50)
        {
            coor = new Coordinaten();
            Coordinaten = coor.GetCoordinatens();
        }
        public Robot(IntPtr _texture, int x, int y, char richtung) : base(_texture, x, y, 50, 50)
        {
            this.richtung = richtung;
            speed = 0.03f;
        }

        public Robot(IntPtr _texture, int x, int y) : base(_texture, x, y, 50, 50)
        {
        ...
        public void CreateEnemy(List<PPong.GUI.Sprite.Sprite> sprites)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                while (!ok)
                {
                    ...
                    {
                        robot = new Robot(_texture, c.getX(), c.getY(), randomChar);
                        if (!robot.IsCollidingWith(s))
                            ok = true;
                        else
                        {
                            ok = false;
